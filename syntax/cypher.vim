...
syn keyword cypherKeyword index unique constraint on for require exists skip
...
syn match cypherFunctionCall  "\zs[A-Za-z0-9_.]\+\ze(" contains=cypherFunction
syn match cypherComment       "\s*\/\/.*$"
syn match cypherNumber        "-\=\<\d*\.\=[0-9_]\>"
syn match cypherLabel         ":\s*\zs\(\w\|_\)\+\ze"
syn match cypherSymbol        "-->\ze("
syn match cypherSymbol        ")\zs<--"
syn match cypherSymbol        "]->\ze("
...
hi def link cypherKeyword       Keyword
hi def link cypherFunction      Function
hi def link cypherFunctionCall  Function
hi def link cypherComment       Comment
hi def link cypherNumber        Number
hi def link cypherOperator      Keyword
