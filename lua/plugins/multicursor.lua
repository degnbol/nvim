...
return {
    {
        ...
        config = function()
            ...
            map.desc('n', "<C-Down>", "Multi cursor down")
            map.desc('n', "<C-Up>", "Multi cursor up")
        end,
    },
    {
    ...
    },
    {
        ...
        config = function()
            ...
            map({ "n", "v" }, "<S-up>", util.fncount(mc.lineAddCursor, -1))
            map({ "n", "v" }, "<S-down>", util.fncount(mc.lineAddCursor, 1))
            map({ "n", "v" }, "<S-A-up>", util.fncount(mc.lineSkipCursor, -1))
            map({ "n", "v" }, "<S-A-down>", util.fncount(mc.lineSkipCursor, 1))

            -- Add or skip adding a new cursor bymatching word/selection
            map({ "n", "v" }, "<C-n>", util.fncount(mc.matchAddCursor, 1))
            map({ "n", "v" }, "<C-S-n>", util.fncount(mc.matchSkipCursor, 1))
            map({ "n", "v" }, "<C-p>", util.fncount(mc.matchAddCursor, -1))
            map({ "n", "v" }, "<C-S-p>", util.fncount(mc.matchSkipCursor, -1))
            ...
            map({ "n", "v" }, "<S-left>", util.fncount(mc.prevCursor), { desc = "Multicursor rotate previous" })
            map({ "n", "v" }, "<S-right>", util.fncount(mc.nextCursor), { desc = "Multicursor rotate next" })
            ...
            map({ "n", "v" }, "<leader>mc", mc.duplicateCursors, { desc = "Clone+lock cursors" })
            ...
            map.v("<leader>ma", mc.alignCursors, "Align")

            -- Split visual selections by regex.
            map.v("<leader>ms", mc.splitCursors, "Split by regex")

            -- Append/insert for each line of visual selections.
            map.v("I", mc.insertVisual, "Insert (multi)")
            map.v("A", mc.appendVisual, "Append (multi)")
