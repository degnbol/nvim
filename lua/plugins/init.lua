...
return {
    -- enables dot-repeat for plugin <Plug> maps via repeat#set(); no effect on native `.`.
    { "vim-repeat" },
    -- vim-abolish provides:
    --   :Subvert (alias :S) — case-preserving substitution (foo/Foo/FOO → bar/Bar/BAR)
    --   Coercion mappings: crs (snake_case), crm (MixedCase), crc (camelCase),
    --                      cru (UPPER_CASE), cr- (dash-case), cr. (dot.case),
    --                      cr<SPACE> (space case), crt (Title Case)
    -- :Abolish (iabbrev generator) is no longer used here — see plugin/abbreviations.lua
    -- which calls our own utils.iabbrev module instead.
    { "vim-abolish" },
