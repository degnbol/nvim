...
function M.match_oneAhead(line_to_cursor, trigger)
    -- look for match which ends one ahead of the cursor.
    -- Note that it doesn't replace the char one ahead, it just requires it in the pattern.
    ...
    local find_res = { line:sub(1, col+1):find(trigger .. "$") }
