...
vim.keymap.set('n', '<localleader>v', vnewFurther,     { buffer=true, desc="vnew with one window height offset and scrollbind enabled. Accepts a count." })
vim.keymap.set('n', '<localleader>f', forwardScreen,   { buffer=true, desc="Like <C-f> but jumps foward one screen visually in the split regardless of scrolloff." })
vim.keymap.set('n', '<localleader>b', backwardScreen,  { buffer=true, desc="Like <C-b> but jumps backwards one screen visually in the split regardless of scrolloff." })
vim.keymap.set('n', '<localleader>c', prettyChordLine, { buffer=true, desc="Write a pretty print of chord at current line." })
vim.keymap.set('n', '<localleader>t', tabNew,          { buffer=true, desc="New tab" })
